...
def plot_analysis(csv_file):
    ...
    ax1.plot(opd_inj_negative_side, color = "b", linewidth=10, alpha=0.25, label="Retrieved")
    ...
    #ax2.set_title("Tip, wrapped (y)", fontsize = 18.0)
    ax2.set_xlim([0,10000])
    ax2.set_ylim([np.nanmin(tip_inj_unwrapped),np.nanmax(tip_inj_unwrapped)])
    ...
    #ax3.set_title("Tilt, wrapped (x)", fontsize = 18.0)
    ax3.set_xlim([0,10000])
    ax3.set_ylim([np.nanmin(tilt_inj_unwrapped),np.nanmax(tilt_inj_unwrapped)])
    ax3.set_ylabel("Tilt (x),\nabsolute (mas)", fontsize = 15.0)
    ax3.axes.get_xaxis().set_visible(False)
    ax3.tick_params(labelsize=14)
